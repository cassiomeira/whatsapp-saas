...
const assertApiKey = () => {
  if (!ENV.forgeApiKey) {
    throw new Error("FORGE_API_KEY is not configured (set BUILT_IN_FORGE_API_KEY)");
  }
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  console.log("[LLM] invokeLLM chamado");
  assertApiKey();
  console.log("[LLM] API key verificada");
  ...
  const model = (ENV.forgeModel && ENV.forgeModel.trim().length > 0)
    ? ENV.forgeModel
    : "gemini-2.5-flash";
  
  const apiUrl = resolveApiUrl();
  console.log("[LLM] Modelo:", model);
  ...
  if (!("max_tokens" in payload)) {
    ...
    payload.max_tokens = requestedMax ?? 1024;
  }
